...
export default function FirstStep() {
    ...
    const handleSuccess = async (data) => {
        try {
            const response = await fetch("http://localhost:8888/api/companies/register", {
                method: "POST",
            ...
            if (response.ok) {
                ...
                await router.push("/account-creation/activation-email");
            } else {
                // Gérer les erreurs de l'API
                const errorData = await response.json();
                // Vous pouvez retourner ces erreurs pour les afficher dans le formulaire
                return {
                    apiError: true,
